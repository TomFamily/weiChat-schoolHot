...
Page({
  ...
  jump: function (e) {
    var id = e.target.id
    console.log("我是回答id：" + id)
    wx.navigateTo({
      url: '/pages/index/rebanDetail/huifu/huifu?articleID=' + id,
    })
  },
    ...
    if (this.data.nextUrl != null) {
      wx.request({
        ...
        success: (res) => {
          ...
          // if (this.data.questionInfo.is_collect == 0) {
          //   // 没有关注过
          //   this.setData({
          //     isAttention: "关注话题"
          //   })
  ...
  ykMethod: function () {
    ...
    // 获取问题详情
    wx.request({
      url: app.globalData.ykUrl + "question?question=" + this.data.questionID + "&type=" + this.data.sortNumber,
      header: {
        Authorization: app.globalData.token,
      },
      dataType: 'json',
